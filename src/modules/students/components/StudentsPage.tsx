...
import { Plus, Search, Edit, Trash2, School, GraduationCap, Users, AlertCircle, Filter, Download, Upload, RefreshCw } from 'lucide-react';
...
const StudentsPage: React.FC = () => {
  ...
  // Cargar todos los estudiantes para búsqueda global
  ...
  const handleStudentSelected = (student: Student) => {
    // Cerrar modal de búsqueda global primero
    setSearchModalOpen(false);
    
    // Abrir el modal de edición directamente
    handleOpenModal('edit', student);
    
    // Si el estudiante seleccionado pertenece a un curso, navegar a ese curso
    if (student.course && student.course.school) {
      // Primero seleccionar el establecimiento
      const school = schools.find(s => s.id === student.course?.school?.id);
      if (school) {
        setSelectedSchool(school);
        
        // Luego cargar los cursos de ese establecimiento y seleccionar el curso
        fetchSchoolCourses(school.id).then(fetchedCourses => {
          setCourses(fetchedCourses);
          const course = fetchedCourses.find(c => c.id === student.course_id);
          if (course) {
            setSelectedCourse(course);
            // En lugar de establecer el nombre completo, solo establecemos el apellido
            // que es más probable que coincida con el filtro
            setSearchTerm(student.last_name);
          }
        });
      }
    }
  ...
  const handleDeleteStudent = async (id: number) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este estudiante?')) {
      try {
        ...
        // Recargar la lista completa de estudiantes
        loadAllStudents();
  ...
  const handleSearch = () => {
    ...
    // Si hay un término de búsqueda pero no hay resultados en el curso actual,
    // abrir el modal de búsqueda global
    if (searchTerm && selectedCourse && filteredStudents.length === 0) {
      setSearchModalOpen(true);
    }
  ...
  const handleExportCsv = () => {
    ...
    link.href = url;
    link.setAttribute('download', `estudiantes-${selectedCourse?.name || 'todos'}.csv`);
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="flex items-center px-3 py-2 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors"
            >
            ...
            {selectedCourse && (
              <>
                <button
                  ...
                  className="flex items-center px-3 py-2 bg-blue-700 text-white font-medium rounded-lg hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors"
                >
                ...
                <button
                  ...
                  className="flex items-center px-3 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button
                  onClick={() => loadStudents(selectedCourse.id)}
                  className="flex items-center px-3 py-2 bg-gray-500 text-white font-medium rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors"
                >
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => handleOpenModal('create')}
                  className="flex items-center px-3 py-1 bg-blue-600 text-white text-sm font-medium rounded hover:bg-blue-700"
                >
            ...
            {/* Barra de búsqueda para estudiantes (ahora ubicada aquí como solicitaste) */}
            ...
            {searchTerm && (
              <div className="bg-blue-50 p-2 rounded-lg text-sm mt-2">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex items-center justify-end space-x-2">
                              <button
                                ...
                                className="flex items-center p-1 text-yellow-600 hover:text-yellow-900 hover:bg-yellow-50 rounded"
                                disabled={isDeleting}
                              ...
                              <button
                                ...
                                className="flex items-center p-1 text-red-600 hover:text-red-900 hover:bg-red-50 rounded"
                                disabled={isDeleting}
      ...
      {modalOpen && (
        <StudentModal
          ...
          onSave={() => {
            ...
            // También actualizar la lista global de estudiantes
            loadAllStudents();
            handleCloseModal();
          }}
        />
